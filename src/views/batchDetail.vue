...
// 🔹 Data Batch Dummy (bisa diganti dari API)
const batch = ref({
  id: 1,
...
onMounted(() => {
  totalMaterial.value = materialList.value.reduce(
    (sum, m) => sum + m.Qty * m.harga_satuan,
    0
  )

  // 🔹 Grafik Fase (Planlet - G0 - G1 - G2)
...
// 🔹 Export ke Excel (termasuk statistik & tabel)
...
<template>
  <div id="exportArea" class="min-h-screen bg-[#FFFD8F] px-6 py-10 text-[#2F5320]">
